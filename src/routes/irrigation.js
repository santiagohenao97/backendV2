...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        let programStatusNuevoTimestamp = new Date();
        let timestampString = programStatusNuevoTimestamp.toISOString();
        const params = {
            "advanced": false,
            ...
            "programStatusNuevoTimestamp": timestampString,
...
router.put('/:id', async (req, res) => {
    ...
    try {
        ...
        const params = {
            ...
            "ini": new Date(data.irrigation.startTime).toISOString(),
            "numrep": -1,
            "numrepPending": -1,
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        //console.log('Preparing to delete:', `/controller/${process.env.CONTROLLER_ID}/device/${process.env.DEVICE_ID}/program/${precimedId.precimedId}?deleteType=1`);
        //console.log('token', token)
        const precimedData = await DELETEData(
            `/controller/${process.env.CONTROLLER_ID}/device/${process.env.DEVICE_ID}/program/${precimedId.precimedId}?deleteType=1`, 
